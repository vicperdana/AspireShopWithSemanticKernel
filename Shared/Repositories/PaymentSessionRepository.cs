...
public interface IPaymentSessionRepository
{
    ...
    Task<PaymentSession?> GetByOrderIdAsync(string orderId, CancellationToken cancellationToken = default);
...
public class PaymentSessionRepository : IPaymentSessionRepository
{
    ...
    public async Task<PaymentSession?> GetByOrderIdAsync(string orderId, CancellationToken cancellationToken = default)
    {
        return await _context.PaymentSessions
            .Include(ps => ps.Order)
